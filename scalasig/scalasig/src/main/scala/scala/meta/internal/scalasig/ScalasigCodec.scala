  ...
  def fromClassfile(classfile: Classfile): Option[Scalasig] = {
    classfile.payload match {
      case ScalaPayload(bytes) =>
        Some(fromBytes(classfile.name, classfile.source, bytes))
      case _ =>
        None
    }
  }

  def fromBytes(fileName: String, fileSource: String, bytes: Array[Byte]): Scalasig = {
    ...
    Scalasig(fileName, fileSource, entries)
  }

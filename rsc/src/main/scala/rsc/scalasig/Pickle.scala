...
class Pickle private (settings: Settings, mtab: Mtab, sroot1: String, sroot2: String) {
  ...
  object Transients {
    ...
    def svalField(sgetterSym: String): s.SymbolInformation = {
      ...
      val tspec = sgetterSym.owner.isTrait && noGetter
      val sfieldName = {
      ...
      val sfieldSig = {
        sgetterSym.ssig match {
          case NullaryMethodSig(stpe) if tspec =>
            s.MethodSignature(None, Nil, stpe)
          case NullaryMethodSig(stpe) =>
            s.ValueSignature(stpe)
    ...
    def scaseAccessor(sgetterSym: String): s.SymbolInformation = {
      val saccessorName = {
        ...
        if (settings.abi == Abi211)
          gensym.caseAccessor(sgetterSym.desc.value)
//        else sgetterSym.desc.value + "$access$" + s"$acc"
        else
          gensym.caseAccessor(sgetterSym.desc.value)
      }
